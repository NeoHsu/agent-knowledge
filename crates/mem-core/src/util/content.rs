...
pub fn validate_memory_resource_limits(
        ...
        || name.chars().any(char::is_control)
    {
        return Err(error::usage(format!(
            "memory name must be between 1 and {MAX_MEMORY_NAME_BYTES} bytes"
        )));
    }
    if description.is_some_and(|value| value.len() > MAX_MEMORY_DESCRIPTION_BYTES) {
        return Err(error::usage(format!(
            "memory description exceeds {MAX_MEMORY_DESCRIPTION_BYTES} bytes"
        )));
    }
    if why.is_some_and(|value| value.len() > MAX_MEMORY_DESCRIPTION_BYTES) {
        return Err(error::usage(format!(
            "memory why exceeds {MAX_MEMORY_DESCRIPTION_BYTES} bytes"
        )));
    }
    ...
    if tags.len() > MAX_MEMORY_TAGS_BYTES {
        return Err(error::usage(format!(
            "memory tags exceed {MAX_MEMORY_TAGS_BYTES} bytes"
        )));
    }
    ...
    if tag_count > MAX_MEMORY_TAG_COUNT {
        return Err(error::usage(format!(
            "memory tags cannot exceed {MAX_MEMORY_TAG_COUNT} entries"
        )));
    }
    if scope.len() > MAX_MEMORY_SCOPE_BYTES {
        return Err(error::usage(format!(
            "memory scope exceeds {MAX_MEMORY_SCOPE_BYTES} bytes"
        )));
    }
    Ok(())
}
...
pub fn required_content(content: Option<String>, content_file: Option<&Path>) -> Result<String> {
    optional_content(content, content_file)?
        .ok_or_else(|| error::usage("one of --content or --content-file is required"))
}
...
pub fn optional_content(
    ...
    match (content, content_file) {
        (Some(_), Some(_)) => Err(error::usage("use only one of --content or --content-file")),
        ...
        (None, Some(path)) => {
            ...
            if bytes > MAX_MEMORY_CONTENT_BYTES {
                return Err(error::usage(format!(
                    "memory content file exceeds {MAX_MEMORY_CONTENT_BYTES} bytes"
                )));
            }
