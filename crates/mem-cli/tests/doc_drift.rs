...
fn render_surface(command: &clap::Command, path: &str, output: &mut String) {
    ...
    for argument in command.get_arguments() {
        ...
        if argument.is_hide_set()
            || id == "help"
            || (id == "version" && path == "mem")
            || (path != "mem" && argument.is_global_set())
        {
            continue;
        }
        ...
        if argument.is_positional() {
            let mut rendered = format!("<{value_name}>");
            if repeated {
                rendered.push_str("...");
            }
        ...
        if !defaults.is_empty() {
            rendered.push_str(&format!("{{default={}}}", defaults.join(",")));
        }
        ...
        if !conflicts.is_empty() {
            rendered.push_str(&format!("{{conflicts={}}}", conflicts.join(",")));
        }
        if !argument.is_required_set() {
            rendered = format!("[{rendered}]");
        }
...
fn cli_surface_snapshot_matches_clap() {
    ...
    let mut actual = String::from(
        "# Generated by `UPDATE_CLI_SURFACE=1 cargo test -p mnemark --test doc_drift cli_surface_snapshot_matches_clap`.\n\
         # One line per command. Hidden compatibility flags are intentionally omitted.\n",
    );
    render_surface(&command, "mem", &mut actual);

    let path = repository_root().join("docs/cli-surface.txt");
...
fn agent_reference_tracks_split_module_paths() {
    ...
    for relative in [
        ...
        "crates/mem-cli/src/commands/admin/mod.rs",
    ...
    for removed in [
        ...
        "crates/mem-cli/src/commands/admin.rs",
